...
from modules import (
    ...
    delivery_board, kb_intake, portfolio_digest,
)
...
NAV = {
    ...
    "Portfolio Weekly Digest": portfolio_digest.render,
...
def settings_page():
    ...
    st.divider()
    # --- Seed ------------------------------------------------------------
    # When role PINs are configured, only a verified Admin may reset data.
    if config.ROLE_PINS and st.session_state.get("role") != "Admin":
        st.info("Seeding / reset is restricted to the Admin role.")
        return
    st.warning("Seeding resets ALL data and loads 4 sample projects.")
...
def main():
    ...
    # Optional PIN gate: only active when PM_ROLE_PINS is configured.
    if config.ROLE_PINS.get(role):
        entered = st.sidebar.text_input("Role PIN", type="password", key=f"pin_{role}")
        if entered != config.ROLE_PINS[role]:
            if entered:
                st.sidebar.error("Incorrect PIN — viewing as Customer.")
            else:
                st.sidebar.info("This role requires a PIN.")
            role = "Customer"
